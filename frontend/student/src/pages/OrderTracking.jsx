...
export default function OrderTrackingPage() {
  ...
  if (loading) {
    return (
      <div className="page-container max-w-3xl space-y-6 py-6 sm:py-8 px-3 sm:px-6">
        <div className="card h-64 skeleton bg-slate-100 animate-pulse rounded-2xl" />
      </div>
    );
  }
  ...
  if (!tracking) {
    return (
      <div className="page-container max-w-xl mx-auto text-center py-12 sm:py-16 card space-y-4 my-6 sm:my-8 px-4">
        <span className="text-4xl sm:text-5xl opacity-50">📦</span>
        <h2 className="text-lg sm:text-xl font-bold text-slate-800">Order Tracking Details</h2>
        <p className="text-xs sm:text-sm text-slate-500 max-w-md mx-auto">Your order has been recorded successfully. Check your order history to view live status updates or download your bill receipt.</p>
        <div className="pt-2">
          <Link to="/orders" className="btn btn-primary font-bold text-xs px-5 sm:px-6 py-2.5">
  ...
  return (
    <div className="page-container max-w-3xl py-4 sm:py-8 space-y-4 sm:space-y-6 animate-fade-in px-3 sm:px-6 lg:px-8">
      {/* Header Banner */}
      <div className="card p-4 sm:p-6 bg-slate-900 text-white flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 shadow-lg border border-slate-800 rounded-2xl sm:rounded-3xl">
        <div className="space-y-1">
          <span className={`badge ${isDelivery ? 'badge-orange' : 'bg-blue-600 text-white font-bold border border-blue-500 shadow-sm px-2.5 sm:px-3 py-0.5 sm:py-1 text-[10px] sm:text-xs'}`}>
            {isDelivery ? '🚴 Hostel Room Delivery' : '🏪 Canteen Counter Pickup'}
          </span>
          <h1 className="text-xl sm:text-2xl font-display font-bold text-white">Order #{tracking.orderNumber || 'CB-ORDER'}</h1>
        ...
        {['PENDING_PAYMENT', 'CONFIRMED'].includes(status) && (
          <button
            ...
            disabled={cancelling}
            className="btn btn-secondary bg-rose-600/20 text-rose-300 border-rose-500/30 hover:bg-rose-600/30 text-xs font-bold py-1.5 px-3"
          >
      ...
      {isDelivery && (
        <div className="card p-4 sm:p-5 border-2 border-orange-200 bg-orange-50/50 space-y-2 rounded-2xl">
          <div className="flex flex-col xs:flex-row items-start xs:items-center justify-between gap-2">
            ...
            <span className="badge badge-orange font-bold text-[10px] sm:text-xs self-start xs:self-center">
              Direct Delivery
            </span>
          </div>

          {deliveryBatch && (
            <p className="text-[11px] sm:text-xs text-orange-800 font-medium pt-2 border-t border-orange-200/80">
      ...
      {/* Pickup Info Card */}
      {!isDelivery && (
        <div className="card p-4 sm:p-5 border-2 border-blue-500/40 bg-blue-50/80 space-y-2 rounded-2xl">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-blue-600 text-white flex items-center justify-center text-lg sm:text-xl font-bold shadow-md flex-shrink-0">
              🏪
            </div>
            <div>
              <h3 className="font-bold text-slate-900 text-xs sm:text-sm">Canteen Counter Pickup</h3>
      ...
      {/* Visual Timeline */}
      <div className="card p-4 sm:p-6 space-y-4 sm:space-y-6 rounded-2xl sm:rounded-3xl">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <h2 className="text-sm sm:text-base font-bold text-slate-900 font-display">
            Live Order Timeline
          </h2>
          <span className="text-[10px] sm:text-xs font-bold text-orange-600 bg-orange-50 px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full border border-orange-200">
            Auto 2s Refresh
          </span>
        </div>

        {status === 'CANCELLED' ? (
          <div className="bg-rose-50 border border-rose-200 rounded-2xl p-5 sm:p-6 text-center space-y-1">
            <span className="text-2xl sm:text-3xl">❌</span>
            <h3 className="font-bold text-sm sm:text-base text-rose-900">Order Cancelled</h3>
            <p className="text-xs text-rose-700">This order was cancelled.</p>
          </div>
        ) : (
          <div className="relative pl-7 sm:pl-8 border-l-2 border-slate-200 space-y-6 sm:space-y-8 my-2">
            {steps.map((stepKey, idx) => {
              ...
              return (
                <div key={stepKey} className="relative group">
                  {/* Circle indicator */}
                  <div
                    className={`absolute -left-[37px] sm:-left-[41px] top-0 w-6 h-6 sm:w-7 sm:h-7 rounded-full flex items-center justify-center text-[10px] sm:text-xs font-bold transition-all ${
                      isCurrent
                  ...
                  <div className="space-y-0.5">
                    <h4 className={`text-xs sm:text-sm font-bold flex items-center gap-1.5 sm:gap-2 ${
                      isCurrent
                        ? ['DELIVERED', 'COMPLETED'].includes(status)
                    ...
                    {isCurrent && (
                      ['DELIVERED', 'COMPLETED'].includes(status) ? (
                        <span className="inline-block text-[10px] sm:text-[11px] text-emerald-700 font-bold bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200 mt-1">
                          ✓ Order Delivered & Completed
                        </span>
                      ) : (
                        <span className="inline-block text-[10px] sm:text-[11px] text-orange-600 font-bold bg-orange-50 px-2 py-0.5 rounded-md border border-orange-200 mt-1">
                          In progress...
                        </span>
                      )
                    )}
      ...
      <div className="text-center pt-2">
        <Link to="/orders" className="text-xs sm:text-sm font-bold text-orange-600 hover:text-orange-700">
