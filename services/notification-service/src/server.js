...
async function startServer() {
  try {
    ...
    const server = app.listen(PORT, () => {
      logger.info({
        msg: `CampusBite Notification Service started`,
        port: PORT,
    ...
    notificationWorker.start().catch((err) => 
      logger.warn({ msg: 'Notification worker deferred', err: err.message })
    );
    ...
    logger.error({ msg: 'Failed to start notification service', err });
    process.exit(1);
  }
}
